...
class TestQuestionRequest:
    ...
    def test_valid_request(self):
        """測試有效的請求"""
        request = QuestionRequest(question="如何配對 AirPods？", top_k=1)
...
class TestQuestionResponse:
    ...
    def test_valid_response(self):
        ...
        response = QuestionResponse(
            question="測試問題", answer="測試答案", status="success"
        )
...
class TestSearchRequest:
    ...
    def test_valid_request(self):
        """測試有效的請求"""
        request = SearchRequest(query="搜尋關鍵字", n_results=5)
...
class TestDocumentModels:
    ...
    def test_document_chunk(self):
        """測試文件片段"""
        chunk = DocumentChunk(
            chunk_text="這是一段文字", chunk_embedding=[0.1, 0.2, 0.3]
        )
    ...
    def test_document_with_embedding(self):
        ...
        doc = DocumentWithEmbedding(
            ...
            chunks=[
                DocumentChunk(chunk_text="片段1", chunk_embedding=[0.1, 0.2, 0.3]),
                DocumentChunk(chunk_text="片段2", chunk_embedding=[0.4, 0.5, 0.6]),
            ],
        )
...
class TestAPIKeyModels:
    ...
    def test_api_key_status_response(self):
        """測試 API Key 狀態回應"""
        response = APIKeyStatusResponse(exists=True, masked="sk-1234...")
