...
class QuestionRequest(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "examples": [{"question": "如何配對 AirPods？", "top_k": 1}]
        }
    }
...
class ImageAnnotationRequest(BaseModel):
    ...
    model_config = {"json_schema_extra": {"examples": [{"target_item": "person"}]}}
...
class DetectedObjectResponse(BaseModel):
    ...
    box_2d: List[int] = Field(
        ..., description="2D 邊界框座標 [y_min, x_min, y_max, x_max]"
    )
...
class ImageAnnotationResponse(BaseModel):
    ...
    annotated_images: List[str] = Field(
        default_factory=list, description="標註圖片檔案路徑"
    )
...
class CollectionsResponse(BaseModel):
    ...
    collections: List[CollectionInfo] = Field(
        ..., description="可用的 collections 列表"
    )
...
class SwitchCollectionRequest(BaseModel):
    ...
    collection_name: str = Field(
        ..., min_length=1, description="要切換的 collection 名稱"
    )

    model_config = {
        "json_schema_extra": {"examples": [{"collection_name": "airpods_manual"}]}
    }
