...
# 初始化日誌
setup_logger(name="app", level=settings.log_level, log_file=settings.log_file)
...
async def get_collections():
    ...
    try:
        ...
        collection_info = [
            CollectionInfo(name=col["name"], count=col["count"]) for col in collections
        ]
        ...
        raise HTTPException(
            status_code=500, detail=f"獲取 collections 時發生錯誤：{str(e)}"
        )
...
async def switch_collection(request: SwitchCollectionRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"切換 collection 時發生錯誤：{str(e)}"
        )
    ...
    try:
        ...
        if not embedding_service:
            if not settings.openai_api_key:
                raise HTTPException(
                    status_code=400, detail="OpenAI API Key 未設定，請先設定 API Key"
                )
            embedding_service = EmbeddingService(provider="openai")
        ...
        documents, distances = db_service.query(
            query_embedding=question_embedding, n_results=request.top_k
        )
        ...
        # 生成答案
        answer = llm_service.generate_answer(question=request.question, context=context)
        ...
        raise HTTPException(status_code=500, detail=f"生成答案時發生錯誤：{str(e)}")
...
async def search_documents(request: SearchRequest):
    ...
    try:
        ...
        # 查詢相似文件
        documents, distances = db_service.query(
            query_embedding=query_embedding, n_results=request.n_results
        )

        logger.info(f"搜尋完成，找到 {len(documents)} 個結果")

        return SearchResponse(query=request.query, results=documents, status="success")
...
async def get_api_key_status():
    ...
    return APIKeyStatusResponse(exists=bool(value), masked=masked)
...
async def upload_image_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        if file.content_type not in allowed_types:
            raise HTTPException(
                status_code=400, detail="不支援的檔案類型，僅支援 JPG、PNG、GIF、WEBP"
            )
...
async def annotate_image(
    file: UploadFile = File(...), target_item: str = Form(default="objects")
):
    """
    上傳並標註圖片
    """
    try:
        ...
        detected_objects = annotation_service.detect_objects(
            image_path=str(file_path), target_item=target_item
        )
        ...
        # 轉換為回應格式
        objects_response = [
            DetectedObjectResponse(box_2d=obj.box_2d, label=obj.label)
        ...
        raise HTTPException(status_code=500, detail=f"標註圖片時發生錯誤：{str(e)}")
...
async def get_annotated_image(filename: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"讀取標註圖片時發生錯誤：{str(e)}")
